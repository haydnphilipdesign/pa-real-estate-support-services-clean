...
const GlassCard: React.FC<GlassCardProps> = ({
  ...
  switch (variant) {
    case 'light':
      baseCardStyle += ' bg-white/85 backdrop-blur-md border border-white/20 shadow-2xl color-scheme-light';
      break;
    case 'dark':
      baseCardStyle += ' bg-gray-900/85 backdrop-blur-md border border-white/10 shadow-2xl color-scheme-dark';
      break;
    case 'blue':
      baseCardStyle += ' bg-brand-blue/85 backdrop-blur-md border border-blue-300/20 shadow-2xl color-scheme-dark';
      break;
    case 'default':
    default:
      baseCardStyle += ' bg-white/85 backdrop-blur-md border border-white/20 shadow-2xl color-scheme-light';
  ...
  // Determine title styling based on variant with proper contrast
  const titleStyle = variant === 'dark' || variant === 'blue' 
    ? 'text-xl font-bold mb-3 break-words text-white' 
    : 'text-xl font-bold mb-3 break-words text-brand-navy';
  
  // Determine subtitle styling based on variant with proper contrast
  const subtitleStyle = variant === 'dark' || variant === 'blue'
    ? 'text-sm mb-4 text-blue-200' 
    : 'text-sm mb-4 text-brand-blue';
