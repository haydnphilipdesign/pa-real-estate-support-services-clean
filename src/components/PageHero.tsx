...
// Use the standardized variants from heroAnimations, optionally enhanced with provided options
const getVariants = (customVariants?: PageHeroProps['motionVariants']) => {
  return {
    container: { 
      ...HERO_ANIMATION.variants.container, 
      ...customVariants?.container 
    },
    title: { 
      ...HERO_ANIMATION.variants.title, 
      ...customVariants?.title 
    },
    subtitle: { 
      ...HERO_ANIMATION.variants.subtitle, 
      ...customVariants?.subtitle 
    },
    content: { 
      ...HERO_ANIMATION.variants.cta, 
      ...customVariants?.content 
    }
  };
};
...
const PageHero: React.FC<PageHeroProps> = ({
  ...
  showScrollIndicator = false, // Default to false to prevent scroll indicator
  motionVariants,
  children
}) => {
  ...
  const contentAlignmentClasses = {
    top: 'items-start pt-20', // Reduced from pt-32
    center: 'items-start pt-20', // Changed from items-center to reduce space
    bottom: 'items-end pb-20' // Reduced from pb-32
  };

  // Get standardized animation variants
  const variants = getVariants(motionVariants);
  ...
  return (
    <section
      ...
      style={{ marginTop: '80px', minHeight: 'calc(100vh - 100px)' }} /* Added to reduce space */
    >
