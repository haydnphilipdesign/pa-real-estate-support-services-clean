...
const ContentCard: React.FC<ContentCardProps> = ({
  ...
  if (cardStyle !== 'default') {
    ...
    if (heroStyle) {
      ...
      baseCardStyle += ' bg-white border border-gray-100 p-6 sm:p-8 shadow-md hover:shadow-lg rounded-2xl';
    }
  }

  // Add overflow controls when in fixed height situations
  if (className && className.includes('max-h-')) {
    baseCardStyle += ' overflow-y-auto scrollbar-hide';
  }
  ...
  // Data attributes to help with targeting in CSS
  const dataAttributes = {
    ...(inHero || heroStyle ? { 'data-hero-component': 'true' } : {}),
    ...(isGlassCard ? { 'data-glass-card': 'true' } : {})
  };
