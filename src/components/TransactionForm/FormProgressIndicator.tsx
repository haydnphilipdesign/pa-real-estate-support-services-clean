...
export function FormProgressIndicator({ 
  ...
  return (
    <div className="relative w-full mb-6 p-3 md:p-4 bg-gradient-to-r from-blue-900/30 to-blue-800/30 rounded-lg shadow-inner overflow-hidden">
      ...
      <div className="sm:hidden mt-3 h-2 bg-white/10 rounded-full overflow-hidden">
        <motion.div 
          initial={{ width: 0 }}
          animate={{ 
            width: `${((currentStep - 1) / (steps.length - 1)) * 100}%`,
            transitionEnd: { width: 'auto' } // Prevent animation glitches
          }}
          transition={{ duration: 0.5, ease: "easeInOut" }}
          className={cn(
            "h-full rounded-full min-w-[8px] transition-all duration-300",
            "relative after:content-[''] after:absolute after:inset-0 after:bg-white/20 after:animate-pulse",
