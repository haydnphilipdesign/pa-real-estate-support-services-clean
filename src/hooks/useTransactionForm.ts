...
// Initial document items based on role
const getInitialDocuments = (role: string = 'BUYERS AGENT'): DocumentItem[] => {
  const coreDocuments = [
    { name: "Agreement of Sale", required: true, selected: false },
    { name: "Deposit Money Notice", required: true, selected: false },
    { name: "Cooperating Broker's Compensation", required: true, selected: false },
    { name: "KW Wire Fraud Notice", required: true, selected: false },
  ];

  const buyerDocuments = [
    { name: "Buyer's Agency Contract", required: true, selected: false },
    { name: "Consumer Notice", required: true, selected: false },
    { name: "Buyer's Estimated Costs", required: true, selected: false },
  ];

  const sellerDocuments = [
    { name: "Listing Agreement", required: true, selected: false },
    { name: "Seller's Property Disclosure", required: true, selected: false },
    { name: "Seller's Estimated Costs", required: true, selected: false },
  ];

  const dualAgentDocuments = [
    { name: "Dual Agency Disclosure", required: true, selected: false },
  ];

  if (role === 'BUYERS AGENT') {
    return [...coreDocuments, ...buyerDocuments];
  } else if (role === 'LISTING AGENT') {
    return [...coreDocuments, ...sellerDocuments];
  } else if (role === 'DUAL AGENT') {
    return [...coreDocuments, ...buyerDocuments, ...sellerDocuments, ...dualAgentDocuments];
  }
...
export const useTransactionForm = () => {
  ...
  // Update document list when agent role changes
  useEffect(() => {
    if (agentData.role) {
      setDocumentsData(prev => ({
        ...prev,
        documents: getInitialDocuments(agentData.role)
      }));
    }
  }, [agentData.role]);

  // Setup proper client type based on agent role
  useEffect(() => {
    if (agentData.role && clients.length > 0) {
      const clientType = agentData.role === "LISTING AGENT" ? "SELLER" : "BUYER";
      const needsUpdate = clients.some(client => client.type !== clientType && agentData.role !== "DUAL AGENT");
  ...
  useEffect(() => {
    try {
      ...
      if (savedDraft) {
        ...
        // Validate that the draft has the expected structure before applying it
        if (parsedDraft && typeof parsedDraft === 'object') {
          // Set all the form state from the draft, using nullish coalescing to fall back to defaults
          if (parsedDraft.agentData) setAgentData(parsedDraft.agentData);
          if (parsedDraft.propertyData) setPropertyData(parsedDraft.propertyData);
          if (parsedDraft.clients && Array.isArray(parsedDraft.clients)) setClients(parsedDraft.clients);
          if (parsedDraft.commissionData) setCommissionData(parsedDraft.commissionData);
          if (parsedDraft.propertyDetails) setPropertyDetails(parsedDraft.propertyDetails);
          if (parsedDraft.titleData) setTitleData(parsedDraft.titleData);
          if (parsedDraft.additionalInfo) setAdditionalInfo(parsedDraft.additionalInfo);
          if (parsedDraft.signatureData) setSignatureData(parsedDraft.signatureData);
          if (parsedDraft.documentsData) setDocumentsData(parsedDraft.documentsData);
          if (parsedDraft.currentStep && typeof parsedDraft.currentStep === 'number') {
            setCurrentStep(parsedDraft.currentStep);
          }

          // Format save time if available
          const lastSaveTime = localStorage.getItem('lastManualSave');
          const timeDisplay = lastSaveTime 
          ...
          toast({
            ...
            description: `Your saved progress from ${timeDisplay} has been restored.`,
            variant: "default",
            duration: 4000,
          });
        } else {
          console.error('Invalid draft structure:', parsedDraft);
          toast({
            title: "Error Loading Draft",
            description: "The saved draft appears to be corrupt. Starting with a new form.",
            variant: "destructive",
          });
      ...
      toast({
        title: "Error Loading Draft",
        description: "There was a problem loading your saved draft. Starting with a new form.",
        variant: "destructive",
      });
      localStorage.removeItem('transactionFormDraft');
    }
  ...
  useEffect(() => {
    // Only save if the form has been interacted with (if agent role is set)
    if (agentData.role) {
      const formData = {
        agentData,
        propertyData,
  ...
  const resetForm = () => {
    ...
    setDocumentsData({ documents: getInitialDocuments(), confirmDocuments: false });
